...
-- ---------- unit/guard site assignments (many-to-many) ----------
-- Which sites a guard/unit CAN be posted to. Separate from units.post below, which is the ONE
-- site they're currently working during their active shift — self-selectable by the guard from
-- their own sidebar ("My Site"), or set by a supervisor from the Units tab. A unit with no rows
-- here isn't restricted to anything in particular (the app treats "no assignments configured
-- yet" as "any site"), so existing rosters keep working exactly as before until a supervisor
-- opts a guard into specific sites.
create table if not exists unit_sites (
    callsign text not null references units(callsign) on delete cascade,
    post_id text not null references posts(id) on delete cascade,
    assigned_at timestamptz not null default now(),
    primary key (callsign, post_id)
  );
create index if not exists unit_sites_callsign_idx on unit_sites(callsign);
...
alter table unit_sites enable row level security;
...
declare t text;
begin
  for t in select unnest(array['users','units','posts','unit_sites','checkpoints','checkpoint_scans','guard_locations','calls','call_supplements',
...
declare t text;
begin
  for t in select unnest(array['units','posts','unit_sites','calls','call_supplements','chat_messages','trucks',
